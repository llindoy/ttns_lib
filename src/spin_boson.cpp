...
void check_discretisation(const std::vector<real_type>& wk, const std::vector<real_type>& gk, const std::shared_ptr<bath::continuous_bath<real_type>>& bath, std::ostream& os, real_type wmin, real_type wmax, size_t nhist = 50)
{
    os << "Discretised bath properties:" << std::endl;
    os << "w_k g_k" << std::endl;

    for(size_t i = 0; i < wk.size(); ++i)
    {
        os << "( "<< wk[i] << ", " << gk[i] << ")" << std::endl;
    }

    std::vector<real_type> hist(nhist);
    real_type dw = (wmax - wmin)/nhist;

    for(size_t i = 0; i < wk.size(); ++i)
    {
        if(wk[i] >= wmin && wk[i] < wmax)
        {
            size_t ind = (wk[i] - wmin)/dw;
            hist[ind] += dw*gk[i]*gk[i]*4*std::atan(1);
        }   
    }
    
    os << "Histogram: " << std::endl;
    os << "w hist(w) S(w)" << std::endl;
    if(bath->nonzero_temperature())
    {
        for(size_t i = 0; i < hist.size(); ++i)
        {
            real_type w = wmin + i*dw + dw/2.0;

            os << w << " " << hist[i] << " " << (bath->S(w) + bath->J(w)) << std::endl;
        }
    }
    else
    {
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        bool check_bath_discretisation = false;
        CALL_AND_HANDLE(IOWRAPPER::load_optional<bool>(doc, "checkbathdiscretisation", check_bath_discretisation), "Failed to load the minimum number of unoccupied elements.");
        bool run_dynamics = !check_bath_discretisation;
        ...
        if(check_bath_discretisation)
        {
            CALL_AND_HANDLE(IOWRAPPER::load_optional<real_type>(doc, "bathhistmin", wmin), "Failed to load the minimum number of unoccupied elements.");

            CALL_AND_HANDLE(IOWRAPPER::load_optional<real_type>(doc, "bathhistmax", wmax), "Failed to load the minimum number of unoccupied elements.");

            size_type nhist = std::sqrt(N);
            CALL_AND_HANDLE(IOWRAPPER::load_optional<size_type>(doc, "bathhistn", nhist), "Failed to load the minimum number of unoccupied elements.");

           if(has_ofile)
            {
                std::ofstream ofs(ofilename.c_str());
                check_discretisation(_wk, _gk, exp, ofs, wmin, wmax, nhist);
            }
            else
            {
                check_discretisation(_wk, _gk, exp, std::cout, wmin, wmax, nhist);
            }
        }
